...
use super::config::{FlexDirection, Pane, Session};
...
impl<R: CmdRunner> Rmx<R> {
    ...
    pub(crate) fn session_to_yaml(&self) -> Result<(), Box<dyn Error>> {
        ...
        let tokens = super::parser::parse(&res);
        log::debug!("tokens: {:#?}", tokens);

        let session = super::config::session_from_tokens(&name, &tokens);
...
mod test {
    ...
    fn stop_session() {
        ...
        let res = rmx.session_stop(&Some(session_name.to_string()));
        let cmds = rmx.cmd_runner().cmds().borrow();
        println!("{:?}", cmds);
        match res {
            Ok(_) => {
