use crate::{
    common::mux::Multiplexer,
    tmux::{parse, Dimensions},
};
...
use crate::{
    ...
    common::path::{find_config, home_dir, resolve_symlink, sanitize_path, to_absolute_path},
    tmux::Target,
};
...
impl<M: Multiplexer> SessionManager<M> {
    ...
    pub(crate) fn start(
        ...
        let dimensions = self.multiplexer.get_dimensions()?;
        ...
        self.multiplexer.create_session(&session.name, &path)?;
        self.multiplexer.setenv(
            &Target::new(&session.name),
            LAIO_CONFIG,
            config.to_str().unwrap(),
        );

        self.multiplexer.flush_commands()?;

        self.process_windows(&session, &dimensions, skip_startup_cmds)?;

        self.multiplexer.bind_key(
            "prefix M-l",
        ...
        if !skip_attach {
            if self.multiplexer.is_inside_session() {
                self.multiplexer.switch_client(session.name.as_str())?;
            } else {
                self.multiplexer.attach_session(session.name.as_str())?;
            }
        }
    ...
    pub(crate) fn stop(
        ...
        let current_session_name = self.multiplexer.current_session_name()?;
        log::trace!("Current session name: {}", current_session_name);

        if !stop_all && name.is_none() && !self.multiplexer.is_inside_session() {
        ...
        if !self.multiplexer.session_exists(&name) {
        ...
        let result = (|| -> Result<()> {
            if !skip_shutdown_cmds {
                ...
                match self.multiplexer.getenv(&Target::new(&name), LAIO_CONFIG) {
                    Ok(config) => {
    ...
    pub(crate) fn to_yaml(&self) -> Result<String> {
        ...
        let layout: String = self.multiplexer.session_layout()?;
        let name: String = self.multiplexer.session_name()?;
        let path: String = self
            .multiplexer
            .session_start_path()?
            .replace(&home_dir, "~");
        let pane_paths = self.multiplexer.pane_paths()?;

        let cmd_dict = self.multiplexer.pane_command()?;
    ...
    fn process_windows(
        ...
        let base_idx = self.multiplexer.get_base_idx()?;
        ...
        session
            ...
            .try_for_each(|(i, window)| -> Result<()> {
                ...
                let window_id = if idx == base_idx {
                    let id = self.multiplexer.get_current_window(&session.name)?;
                    self.multiplexer
                ...
                self.multiplexer.select_custom_layout(
    ...
    fn generate_layout(
        ...
        for (index, pane) in panes.iter().enumerate() {
            ...
            if let Some(style) = &pane.style {
                self.multiplexer.set_pane_style(
                    &Target::new(session_name)
                        .window(window_id)
                        .pane(pane_id.as_str()),
                    style,
            ...
            if !skip_cmds {
                self.multiplexer.register_commands(
                    &Target::new(session_name)
                        .window(window_id)
                        .pane(pane_id.as_str()),
                    &pane.commands,
    ...
    fn try_switch(&self, name: &str, skip_attach: bool) -> Result<bool> {
        if self.multiplexer.session_exists(name) {
            ...
            if !skip_attach {
                if self.multiplexer.is_inside_session() {
                    self.multiplexer.switch_client(name)?;
                } else {
                    self.multiplexer.attach_session(name)?;
                }
            }
            return Ok(true);
        }
